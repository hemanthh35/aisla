// Faculty Chat - Real-time messaging between students and faculty
...
const FacultyChat = () => {
    ...
    useEffect(() => {
        ...
        socket.on('room:status_updated', ({ chatRoom, systemMessage }) => {
            setChatRooms(prev => prev.map(room =>
                room._id === chatRoom._id ? chatRoom : room
            ));
    ...
    const fetchMessages = async (roomId) => {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.get(`/api/faculty-chat/rooms/${roomId}/messages`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleStartChat = async (faculty) => {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post('/api/faculty-chat/rooms', {
    ...
    return (
        <div className="faculty-chat-page">
            ...
            <aside className={`chat-sidebar ${sidebarOpen ? 'open' : ''}`}>
                ...
                {/* New Chat Button (Students only) */}
                {isStudent && (
                    <button className="new-chat-btn" onClick={() => setShowFacultyList(!showFacultyList)}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <circle cx="12" cy="12" r="10" />
                            <line x1="12" y1="8" x2="12" y2="16" />
                            <line x1="8" y1="12" x2="16" y2="12" />
                        </svg>
                        {showFacultyList ? 'Back to Chats' : 'New Chat'}
                    </button>
                )}

                {/* Faculty List (for starting new chats) */}
                {isStudent && showFacultyList && (
                    <div className="faculty-list">
                        ...
                        {filteredFaculty.map(faculty => (
                            ...
                            >
                                <div className={`avatar ${isUserOnline(faculty._id) ? 'online' : ''}`}>
                                    {getInitials(faculty.name)}
                                </div>
                                <div className="faculty-info">
                                    <span className="name">{faculty.name}</span>
                                    <span className="department">{faculty.department || 'General'}</span>
                                </div>
                                <span className={`status-badge ${isUserOnline(faculty._id) ? 'online' : 'offline'}`}>
                    ...
                    <div className="chat-rooms-list">
                        {loading ? (
                            ...
                            <div className="no-chats">
                                ...
                                <p>{isStudent ? 'No chats yet. Start a conversation with a faculty member!' : 'No student chats yet.'}</p>
                            </div>
                        ) : (
                            filteredRooms.map(room => {
                                const otherUser = isStudent ? room.faculty : room.student;
                                const unreadCount = isStudent ? room.unreadCount?.student : room.unreadCount?.faculty;

                                return (
                                    <div
                                        key={room._id}
                                        className={`chat-room-item ${selectedRoom?._id === room._id ? 'active' : ''} ${room.status !== 'active' ? room.status : ''}`}
                                        onClick={() => handleSelectRoom(room)}
                                    >
                                        <div className={`avatar ${isUserOnline(otherUser?._id) ? 'online' : ''}`}>
                    ...
                    <>
                        ...
                        <header className="chat-header">
                            ...
                            <div className="chat-user-info">
                                <div className={`avatar ${isUserOnline(isStudent ? selectedRoom.faculty?._id : selectedRoom.student?._id) ? 'online' : ''}`}>
                                    {getInitials(isStudent ? selectedRoom.faculty?.name : selectedRoom.student?.name)}
                                </div>
                                <div className="user-details">
                                    <span className="user-name">
                                        {isStudent ? selectedRoom.faculty?.name : selectedRoom.student?.name}
                                    </span>
                                    <span className={`user-status ${isUserOnline(isStudent ? selectedRoom.faculty?._id : selectedRoom.student?._id) ? 'online' : ''}`}>
                                        {isUserOnline(isStudent ? selectedRoom.faculty?._id : selectedRoom.student?._id) ? 'Online' : 'Offline'}
                                    </span>
                                </div>
                            </div>
                                ...
                                return (
                                    <React.Fragment key={msg._id}>
                                        ...
                                        {msg.messageType === 'system' ? (
                                            ...
                                            <div className={`message ${isOwnMessage ? 'own' : 'other'} ${msg.messageType}`}>
                                                {!isOwnMessage && (
                                                ...
                                                <div className="message-bubble">
                                                    {msg.messageType !== 'text' && (
                                                        <div className={`message-type-badge ${msg.messageType}`}>
                                                            {msg.messageType === 'doubt' ? '❓ Doubt' : msg.messageType === 'task' ? '📋 Task' : ''}
                                                        </div>
                                                    )}
                                                    ...
                                                    {isFaculty && !isOwnMessage && (
                                                        <>
                                                            {msg.messageType === 'doubt' && !msg.metadata?.isResolved && (
                                                                <button
                                                            ...
                                                            {msg.messageType === 'task' && (
                                                                <div className="task-actions">
                                                                    <select
                                                                        value={msg.metadata?.taskStatus || 'pending'}
                                                                        onChange={(e) => handleUpdateTaskStatus(msg._id, e.target.value)}
                                                                    >
                                                    ...
                                                    {msg.messageType === 'task' && msg.metadata?.taskStatus && (
                                                        <span className={`task-status-badge ${msg.metadata.taskStatus}`}>
                        ...
                        {selectedRoom.status === 'active' && (
                            <form className="message-form" onSubmit={handleSendMessage}>
                                <div className="message-type-selector">
                                    ...
                                    <button
                                        type="button"
                                        className={`type-btn ${messageType === 'doubt' ? 'active' : ''}`}
                                    ...
                                    <button
                                        type="button"
                                        className={`type-btn ${messageType === 'task' ? 'active' : ''}`}
                                ...
                                <div className="input-wrapper">
                                    <input
                                        ...
                                        type="text"
                                        placeholder={
                                            messageType === 'doubt' ? 'Type your doubt...' :
                                                messageType === 'task' ? 'Describe the task...' :
                                                    'Type a message...'
                                        }
                                        value={newMessage}
