...
const requireAdmin = async (req, res, next) => {
    try {
        ...
        const decoded = jwt.verify(token, process.env.JWT_SECRET || 'aisla_secret_key_2024');
...
router.get('/ai', requireAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            ollamaStatus = { available: false, models: [], error: 'Cannot connect to Ollama' };
        }
...
router.put('/ai', requireAdmin, async (req, res) => {
    try {
        const { provider, ollamaModel, ollamaUrl, geminiModel, temperature, maxTokens } = req.body;

        // Validate provider
        if (provider && !['ollama', 'gemini'].includes(provider)) {
            return res.status(400).json({ message: 'Invalid provider. Must be "ollama" or "gemini"' });
        }

        // If switching to Gemini, check if API key exists
        if (provider === 'gemini' && !process.env.GEMINI_API_KEY) {
            return res.status(400).json({
                message: 'Gemini API key not configured. Please add GEMINI_API_KEY to server environment.'
            });
        }
        ...
        console.log(`✅ [SETTINGS] AI provider updated to: ${newSettings.provider} by ${req.user.email}`);
