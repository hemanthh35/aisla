    ...
    io.on('connection', (socket) => {
        ...
        socket.on('room:join', async ({ roomId }) => {
            try {
                ...
                // Update unread count
                const unreadField = userRole === 'student' ? 'unreadCount.student' : 'unreadCount.faculty';
        ...
        socket.on('message:send', async (data) => {
            try {
                const { roomId, content, messageType = 'text', file, metadata, replyTo } = data;
        ...
        socket.on('doubt:resolve', async ({ messageId, roomId }) => {
            try {
                if (userRole === 'student') {
                    return socket.emit('error', { message: 'Only faculty can resolve doubts' });
                }
        ...
        socket.on('task:update_status', async ({ messageId, roomId, status }) => {
            try {
                if (userRole === 'student') {
                    return socket.emit('error', { message: 'Only faculty can update task status' });
                }
        ...
        socket.on('room:update_status', async ({ roomId, status }) => {
            try {
                if (userRole === 'student') {
                    return socket.emit('error', { message: 'Only faculty can update chat status' });
                }
                ...
                const chatRoom = await ChatRoom.findByIdAndUpdate(roomId, updateData, { new: true })
                ...
                io.to(`room:${roomId}`).emit('room:status_updated', { chatRoom, systemMessage });
            } catch (error) {
