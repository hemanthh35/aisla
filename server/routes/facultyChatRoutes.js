...
router.post('/rooms', protect, async (req, res) => {
    try {
        const { facultyId, subject, category } = req.body;
        const studentId = req.user._id;

        if (req.user.role !== 'student') {
            return res.status(403).json({ message: 'Only students can initiate chats' });
        }
        ...
        if (!chatRoom) {
            ...
            const systemMessage = new Message({
                ...
                senderRole: 'system',
                messageType: 'system',
                content: `Chat started with ${faculty.name}`
            });
...
router.patch('/rooms/:roomId/status', protect, async (req, res) => {
    try {
        ...
        if (!chatRoom.faculty.equals(userId) && req.user.role !== 'admin') {
            return res.status(403).json({ message: 'Only faculty can update chat status' });
        }
...
router.get('/rooms/:roomId/messages', protect, async (req, res) => {
    try {
        ...
        const unreadField = req.user.role === 'student' ? 'unreadCount.student' : 'unreadCount.faculty';
...
router.post('/rooms/:roomId/messages', protect, async (req, res) => {
    try {
        ...
        // Update chat room
        const unreadField = userRole === 'student' ? 'unreadCount.faculty' : 'unreadCount.student';
...
router.patch('/messages/:messageId/resolve', protect, async (req, res) => {
    try {
        ...
        if (!chatRoom.faculty.equals(userId) && req.user.role !== 'admin') {
            return res.status(403).json({ message: 'Only faculty can resolve doubts' });
        }
...
router.patch('/messages/:messageId/task-status', protect, async (req, res) => {
    try {
        ...
        const chatRoom = await ChatRoom.findById(message.chatRoom);
        if (!chatRoom.faculty.equals(userId) && req.user.role !== 'admin') {
            return res.status(403).json({ message: 'Only faculty can update task status' });
        }
    ...
    try {
        ...
        const [
            ...
            Message.countDocuments({
                chatRoom: { $in: await ChatRoom.find({ faculty: facultyId }).distinct('_id') },
                messageType: 'doubt',
...
router.get('/dashboard/pending', protect, async (req, res) => {
    try {
        ...
        const facultyId = req.user._id;
        const chatRoomIds = await ChatRoom.find({ faculty: facultyId }).distinct('_id');
