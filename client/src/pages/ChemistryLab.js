...
const ChemistryLab = () => {
  ...
  useEffect(() => {
    ...
    const initScene = async () => {
      ...
      engine = new BABYLON.Engine(canvasRef.current, true, {
        ...
        stencil: true, // Keeping stencil as it might be needed for other effects, but verified it's usually safe
      });
      ...
      // Create environment texture for realistic reflections using Helper (More Robust)
      try {
        ...
        console.warn(
          "Environment load failed, falling back to default light",
          err
        );
      }

      // Enable shadows with high quality
      const shadowGenerator = new BABYLON.ShadowGenerator(1024, spotLight); // Reduced texture size for compatibility
  ...
  const createLabEnvironment = async (scene, shadowGenerator) => {
    // Floor with StandardMaterial instead of GridMaterial to avoid WebGL errors
    ...
    floorMaterial.diffuseColor = new BABYLON.Color3(0.15, 0.15, 0.2);
